...
function PerfectForesight()
    ...
    ## Variables
    @variable(m, pflow[Lines,T])
    @variable(m, pgeneration[Generators,T])
    @variable(m, storage[Nodes,T] >= 0)
    @variable(m, batterycharge[Nodes,T] >= 0)
    @variable(m, batterydischarge[Nodes,T] >= 0)
    @variable(m, loadshedding[Nodes,T] >= 0)
    @variable(m, productionshedding[Nodes,T] >= 0)
    @variable(m, p_in[T])
    @variable(m, p_out[T])
