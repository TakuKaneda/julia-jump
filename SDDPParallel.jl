...
@everywhere function CreateDictionaryV(ArrayChoice)
...
@everywhere function CreateDictionaryA(ArrayChoice)
    Num = length(ArrayChoice);
...
@everywhere function UsefulCut(LayerChoice, TimeChoice, OutcomeChoice, ECut, eCut, Etemp, etemp, NumCuts)
    "Written Daniel/Taku"
    "Function that determines wether the obtained cut is repeated
    ...
    i = 1;
    boo = true;
    while boo == true && i <= NumCuts[OutcomeChoice, TimeChoice, LayerChoice]
...
# The NLDS algorithm
@everywhere function NLDS(TimeChoice, SampleChoice, iter, LayerChoice, OutcomeChoice, storageTrials, ECut, eCut, NumCuts)
    ...
    if TimeChoice == H # no cuts
        ...
        if NumCuts[OutcomeChoice, TimeChoice, LayerChoice] >= 1 # if there exist cuts
            @constraint(model, Cuts[i = 1:NumCuts[OutcomeChoice, TimeChoice, LayerChoice] ],
...
@everywhere function ForwardTrial(iter, LayerChoice, SampleScenario, SampleChoice, pflowTrials, storageTrials, batterychargeTrials, batterydischargeTrials, loadsheddingTrials, productionsheddingTrials, pgenerationTrials, eCut, ECut, NumCuts)
    " LayerChoice -- Refers to the current Layer"
    " iter -- Refers to the current iteration"
    " SampleScenario -- Refers to the array where paths are stored "
    " SampleChoice -- Refers to the current trial solution"
    " Other parameters are just the arrays to store Cuts, Vairables, etc."
...
function ForwardPass(K, LayerChoice, iter)
    ...
    FirstNLDS = NLDS(1, 1, iter, LayerChoice, 1, storageTrials, ECut, eCut, NumCuts);
    #println("\n")
    ...
    pmap(ForwardTrial, [iter for i = 1:K], [LayerChoice for i = 1:K], [SampleScenario for i = 1:K] , 1:K,
    [pflowTrials for i = 1:K], [storageTrials for i = 1:K], [batterychargeTrials for i = 1:K],
    [batterydischargeTrials for i = 1:K], [loadsheddingTrials for i = 1:K], [productionsheddingTrials for i = 1:K],
    [pgenerationTrials for i = 1:K], [eCut for i = 1:K], [ECut for i = 1:K], [NumCuts for i = 1:K])
end

# This is the inside function for the BackwardPass, helpful to paralelize
@everywhere function BackwardTrial(TimeChoice, SampleChoice, iter, LayerChoice, storageTrials, ECut, eCut, NumCuts, ECutTempStorage, eCutTempStorage)
    " Written by Daniel/Taku"
    " TimeChoice -- Refers to the time stage t"
    " SampleChoice -- Refers to the index of the sample of the Monte Carlo"
    " iter -- Refers to the current iteration"
    " LayerChoice -- Refers to the current Layer"
    " storageTrials -- The reference for the array that stores the storage decisions "
    " eCut, ECut -- The reference for the arrays that store the cuts"
    " NumCuts -- Array that sores the number of cuts" # Note: This may be omitted by finding a more elegant way to store Cuts
    ...
    for OutcomeChoice_1 = 1:NLattice[TimeChoice-1]
        ...
        if TimeChoice < H  # if there are cuts
            ee += sum(TransProb[LayerChoice,TimeChoice-1][OutcomeChoice_1,k] * (
                sum(NesLDS_tk[k].Cuts .* eCut[1:NumCuts[k, TimeChoice, LayerChoice], k, TimeChoice, LayerChoice]
                 ) ) for k = 1:NLattice[TimeChoice] )
        end
    ...
    for t = 0:H-2 # for loop of time Stage
        ...
        for SampleChoice = 1:K
            for OutcomeChoice_1 = 1:NLattice[TimeChoice-1]
                ...
                etemp = eCutTempStorage[SampleChoice, OutcomeChoice_1];
                if NumCuts[OutcomeChoice_1, TimeChoice-1, LayerChoice] >= 1  # if there already exist some cuts
                    if UsefulCut(LayerChoice, TimeChoice-1, OutcomeChoice_1, ECut, eCut, Etemp, etemp, NumCuts) # if the cut is useful -> add it
                        NumCuts[OutcomeChoice_1, TimeChoice-1, LayerChoice] += 1;
                        for i = 1:length(LayerNodes[LayerChoice])
                            n = LayerNodes[LayerChoice][i];
                            ECut[NumCuts[OutcomeChoice_1, TimeChoice-1, LayerChoice], n, OutcomeChoice_1, TimeChoice-1] = Etemp[i];
                        end
                        eCut[NumCuts[OutcomeChoice_1, TimeChoice-1, LayerChoice], OutcomeChoice_1, TimeChoice-1, LayerChoice] = etemp;
                    end
